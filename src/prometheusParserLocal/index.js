...
/**
 * TODO: Describe this and convert to typescript
 * @param metrics
 * @param datasetLabelKey a string representing the property in "labels" that groups the datasets, i.e. for services its "api"
 * @returns {*[]}
 */
export default function parsePrometheusTextFormat(metrics, datasetLabelKey) {
...
// Written by Bopmatic team - this parser is very unfriendly to time-series data
function _flattenMetrics(metrics, groupName, keyName, valueName) {
  let flattened = null;
  for (let i = 0; i < metrics.length; ++i) {
    const sample = metrics[i];
    if (sample.labels && sample.labels[keyName] && sample[valueName]) {
      const _key = sample.labels[groupName] + sample['timestamp_ms']; // "GetOrder - 1703929182"
      if (!flattened) {
        flattened = {};
      }
      if (!flattened[_key]) {
      ...
      if (!flattened[_key]['quantiles']) {
        flattened[_key]['quantiles'] = {};
      }
      flattened[_key]['quantiles'][sample.labels[keyName]] = sample[valueName];
      flattened[_key]['timestamp_ms'] = sample['timestamp_ms'];
      flattened[_key]['labels'] = sample['labels'];
      delete flattened[_key]['labels']['quantile'];
    } else {
      // this shouldn't happen with our data ??? TODO: check this
    }
  }
  if (flattened) {
...
// flattenMetrics(samples, 'quantiles', 'quantile', 'value');
// keyName = what you get from the metric labels
